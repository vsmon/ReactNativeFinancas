...
export default function TransactionsChart({data}: IChartProps) {
  const font = useFont(inter, 12);

  function groupByAssetType(data: IValues[]) {
    const totalInflow: number = data.reduce((sum, curr) => {
      if (curr.type === 'inflow') {
        sum += curr.value;
      }
      return sum;
    ...
    //console.log('totalOutflow========', totalOutflow);
    const total: number = data.reduce((sum, curr) => sum + curr.value, 0);
